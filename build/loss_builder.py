...
from losses.task_loss import FocalLoss, BinaryCrossEntropyLoss
from losses.reliability_loss import reliability_loss


def build_losses(cfg, pos_weights):

    loss_cfg = cfg["loss"]

    task_losses = {
        "valence": BCEWithLogitsLoss(pos_weights['valence']),
        "arousal": BCEWithLogitsLoss(pos_weights['arousal']),
    }
